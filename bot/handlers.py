...
async def confirm_assembly(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        if file_size < 50 * 1024 * 1024:  # Telegram-Limit: 50 MB
            await update.message.reply_video(
                video=open(str(result_path), "rb"),
                caption=(
            ...
            await update.message.reply_document(
                document=open(str(result_path), "rb"),
                caption=(
                    f"✅ *Kurban-Video fertig!*\n"
                    f"👤 Spender: `{context.user_data['donor_name']}`\n"
                    f"📁 `{normalized_name}_{counter}`\n\n"
                    f"⚠️ Datei ist zu groß für Video-Vorschau, wird als Dokument gesendet."
                ),
                parse_mode="Markdown",
            )

        logger.info(
            f"Video erfolgreich erstellt: {result_path} "
